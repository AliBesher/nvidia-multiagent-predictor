...
class OpeningPredictionModel:
    """
    Random Forest model for predicting next-day OPENING direction.
    
    Predicts: GAP UP (1) or GAP DOWN (0)
    Label:    1 if next_day_open > close_price, else 0
    
    Same features as PredictionModel but learns opening-gap patterns
    instead of close-to-close patterns.
    """
    
    def __init__(self, model_path: str = "models/trained_opening_model.pkl"):
        self.model_path = model_path
        self.model: Optional[RandomForestClassifier] = None
        self.scaler = StandardScaler()
        self.feature_names = [
        ...
        logger.info(f"OpeningPredictionModel initialized (trained: {self.is_trained})")
    
    def _load_model(self) -> bool:
        """Load trained opening model from disk if exists"""
        try:
            if os.path.exists(self.model_path):
                data = joblib.load(self.model_path)
                self.model = data['model']
                ...
                self.training_samples = data.get('training_samples', 0)
                self.accuracy = data.get('accuracy', 0.0)
                logger.info(f"Loaded trained opening model ({self.training_samples} samples, {self.accuracy:.1%} accuracy)")
                return True
        except Exception as e:
            logger.warning(f"Could not load opening model: {e}")
        return False
    
    def _save_model(self) -> bool:
        """Save trained opening model to disk"""
        try:
            os.makedirs(os.path.dirname(self.model_path), exist_ok=True)
            data = {
                'model': self.model,
            ...
            joblib.dump(data, self.model_path)
            logger.info(f"Opening model saved to {self.model_path}")
            return True
        except Exception as e:
            logger.error(f"Failed to save opening model: {e}")
            return False
    ...
    def prepare_features(self, data: List[Dict]) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        LABEL DIFFERENCE vs PredictionModel:
          - PredictionModel:        label = 1 if next_day_close > close_price
          - OpeningPredictionModel:  label = 1 if next_day_open  > close_price  (GAP UP)
        """
        if len(data) < MIN_TRAINING_SAMPLES:
            logger.warning(f"Opening model: insufficient data: {len(data)} records (need {MIN_TRAINING_SAMPLES})")
            return None, None
        ...
        for i, row in df.iterrows():
            # Skip rows without next_day_open (can't calculate label)
            if row.get('next_day_open') is None or row.get('close_price') is None:
                continue
            
            # LABEL: 1 if next day opens ABOVE today's close (GAP UP), else 0
            next_open = float(row['next_day_open'])
            current_close = float(row['close_price'])
            label = 1 if next_open > current_close else 0
        ...
        if len(features_list) < MIN_TRAINING_SAMPLES:
            logger.warning(f"Opening model: not enough valid samples: {len(features_list)}")
            return None, None
        
        return np.array(features_list), np.array(labels_list)
    
    def train(self, data: List[Dict]) -> Dict:
        """Train the opening prediction model"""
        result = {
            'success': False,
            'samples': 0,
            'accuracy': 0.0,
        ...
        if X is None or y is None:
            result['message'] = f"Insufficient data for opening model training (have {len(data)}, need {MIN_TRAINING_SAMPLES})"
            logger.warning(result['message'])
            return result
        ...
        try:
            ...
            result['samples'] = self.training_samples
            result['accuracy'] = self.accuracy
            result['message'] = f"Opening model trained on {self.training_samples} samples"
            
            # Log feature importance
            importance = dict(zip(self.feature_names, self.model.feature_importances_))
            sorted_importance = sorted(importance.items(), key=lambda x: x[1], reverse=True)
            logger.info("Opening model feature importance:")
            for name, imp in sorted_importance:
                logger.info(f"  {name}: {imp:.3f}")
            ...
            result['message'] = f"Opening model training failed: {str(e)}"
            logger.error(result['message'])
            return result
    ...
    def predict(self, current_data: Dict) -> Dict:
        ...
        if not self.is_trained or self.model is None:
            result['message'] = "Opening model not trained yet"
            return result
        ...
        try:
            ...
            result['prediction'] = 'GAP UP' if prediction == 1 else 'GAP DOWN'
            result['probability_up'] = probabilities[1] if len(probabilities) > 1 else probabilities[0]
            result['probability_down'] = probabilities[0] if len(probabilities) > 1 else 1 - probabilities[0]
            result['confidence'] = max(result['probability_up'], result['probability_down'])
            result['can_predict'] = True
            result['message'] = f"Opening Prediction: {result['prediction']} ({result['confidence']:.1%} confidence)"
            
            return result
            
        except Exception as e:
            result['message'] = f"Opening prediction failed: {str(e)}"
            logger.error(result['message'])
            return result
    
    def get_status(self) -> Dict:
        """Get opening model status summary"""
        return {
            'is_trained': self.is_trained,
            'training_samples': self.training_samples,
            'accuracy': self.accuracy,
